...
public class ToDoListApp {
    ...
    public static void addNewTask(Scanner scanner, List<ToDoV1> tasks) {
        System.out.println("Введите название новой задачи: ");
        String newTaskTitle = scanner.nextLine();
        tasks.add(new ToDoV1(newTaskTitle));
    ...
    public static void markTaskAsDone(Scanner scanner, List<ToDoV1> tasks) {
    ...
    public static List<ToDoV1> loadTasksFromFile(String fileName) {
        List<ToDoV1> tasks = new ArrayList<>();
            ...
            try {
                if (fileName.endsWith(".json")) {
                    tasks = objectMapper.readValue(file,
                            objectMapper.getTypeFactory().constructCollectionType(List.class, ToDoV1.class));
                }
                else if (fileName.endsWith(".bin")) {
                    try (ObjectInputStream ois = new ObjectInputStream(new FileInputStream(file))) {
                        tasks = (List<ToDoV1>) ois.readObject();
                    }
                }
                else if (fileName.endsWith(".xml")) {
                    tasks = xmlMapper.readValue(file,
                            xmlMapper.getTypeFactory().constructCollectionType(List.class, ToDoV1.class));
    ...
    public static void saveTasksToFile(String fileName, List<ToDoV1> tasks) {
        try {
            if (fileName.endsWith(".json")) {
    ...
    public static void displayTasks(List<ToDoV1> tasks) {
        System.out.println("Список задач: ");
        for (int i = 0; i < tasks.size(); i++) {
            ToDoV1 task = tasks.get(i);
